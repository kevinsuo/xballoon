    ...
    struct vcpu *v, int new_state, s_time_t new_entry_time)
{
	...
	/* add by Kun */
	/* if the domain is not idle domain and vcpu current state is not running 
	   and the delta is not due to cap */
        if (!is_idle_domain(v->domain) && v->runstate.state > 0 && delta < 25000000) {
		delta_time = NOW() - v->vcpu_allocate_time;
		if (delta > delta_time) {
			delta = delta_time;
		}

                v->xBalloon_sleep_time += delta;
        }
        /* end */
    }
...
void vcpu_pause_fast(struct vcpu *v)
{

    atomic_inc(&v->pause_count);
    BUG_ON( is_idle_vcpu(v) );

    if (vcpu_info(v, evtchn_upcall_pending) && !vcpu_info(v, evtchn_upcall_mask) && atomic_read(&v->sleep_count))
    {
	    //clear_bit(CSCHED_FLAG_VCPU_PARKED, &svc->flags);
	    SCHED_OP(VCPU2OP(v), clear_flag, v);
	    atomic_dec(&v->sleep_count);
	    atomic_dec(&v->pause_count);
	    return;
...
void vcpu_wake(struct vcpu *v)
{
    ...
    if ( likely(vcpu_runnable(v)) )
    {
        TRACE_3D(TRC_SCHED_WAKE, v->domain->domain_id, v->vcpu_id, cpu);
...
/***********************[begin]*************************************/
    ...
    d1 = (unsigned)(now >> 32);
       // TRACE_6D(TRC_SCHED_IOWAKE, v->domain->domain_id, v->vcpu_id, d1, d2, atomic_read(&v->sleep_count), cpu);
...
/***********************[end]***************************************/
    if ( !test_and_clear_bit(_VPF_blocked, &v->pause_flags) ) {   //add by Jia
         if (v->runstate.state == RUNSTATE_runnable || v->runstate.state == RUNSTATE_running)
	  vcpu_wake(v);	 
         return;
...
void vcpu_block(void)
{
    ...
    /* Check for events /after/ blocking: avoids wakeup waiting race. */
        TRACE_5D(TRC_SCHED_BLOCK, v->domain->domain_id, v->vcpu_id, 0, cpu, vcpu_event_delivery_is_enabled(v));
    ...
    {
        TRACE_5D(TRC_SCHED_BLOCK, v->domain->domain_id, v->vcpu_id, 1, cpu, vcpu_event_delivery_is_enabled(v));
        raise_softirq(SCHEDULE_SOFTIRQ);
    }
}
...
{
    int cpu = smp_processor_id();
    TRACE_5D(TRC_SCHED_BLOCK, current->domain->domain_id, current->vcpu_id, 2, cpu, vcpu_event_delivery_is_enabled(current));
...
static void do_sleep(void)
{
   ...
   // local_events = vcpu_event_delivery_is_enabled(v); 
    //if (local_events_need_delivery())
//	return;
    TRACE_6D(TRC_SCHED_VSLEEP, v->domain->domain_id, v->vcpu_id, cpu, atomic_read(&v->pause_count), d1, d2);
//    TRACE_6D(TRC_SCHED_VSLEEP, v->domain->domain_id, v->vcpu_id, cpu, local_events, d1, d2);
    //if ( sleep.timeout <= 0 ) {
      //  stop_timer(&v->balloon_timer);
    //} else {
...
static long do_poll(struct sched_poll *sched_poll)
{
    ...
    //TRACE_3D(TRC_SCHED_BLOCK, d->domain_id, v->vcpu_id, 555);
...
/***********************[begin]*************************************/    
    ...
    {
        struct vcpu *v= current;
        int state = (int)(v->runstate.xBalloon_state);
        //int state = atomic_read(&v->xBalloon_state);
        ret = state;
        break;
    }
/***********************[end]***************************************/
...
long do_set_timer_op(s_time_t timeout)
{
    ...
    TRACE_3D(TRC_SCHED_SETTIMER, v->domain->domain_id, v->vcpu_id, 0);
    if ( timeout == 0 )
    {
    TRACE_3D(TRC_SCHED_SETTIMER, v->domain->domain_id, v->vcpu_id, 1);
    ...
    TRACE_3D(TRC_SCHED_SETTIMER, v->domain->domain_id, v->vcpu_id, 2);
        set_timer(&v->singleshot_timer, NOW() + MILLISECS(100));
    }
    else
    {
    TRACE_3D(TRC_SCHED_SETTIMER, v->domain->domain_id, v->vcpu_id, 3);
...
//        if(prev->xBalloon_sleep_time >= 29500000*(100-cap)/100) {
    ...
    TRACE_6D(TRC_SCHED_KUN_2, 	prev->domain->domain_id, 
			      	prev->vcpu_id, 
				next->domain->domain_id, 
				next->vcpu_id,
				d1, 
    ...
    TRACE_6D(TRC_SCHED_KUN_3,   prev->domain->domain_id, prev->xBalloon_sleep_time,  prev->runstate.xBalloon_state,  
				next->domain->domain_id, next->xBalloon_sleep_time,  next->runstate.xBalloon_state);
...
//    int cap, capped; 
    ...
    d2 = (unsigned)(now & 0xffffffffLL);
    d1 = (unsigned)(now >> 32);
    //_update_runstate_area(v);
    ret = v->arch.pv_vcpu.need_update_runstate_area;
...
static void poll_timer_fn(void *data)
{
    ...
    TRACE_3D(TRC_SCHED_BLOCK, v->domain->domain_id, v->vcpu_id, 555);
    if ( test_and_clear_bit(v->vcpu_id, v->domain->poll_mask) )
        vcpu_unblock(v);
}
...
static void balloon_timer_fn(void *data)
{
    ...
    d2 = (unsigned)(now & 0xffffffffLL);
    d1 = (unsigned)(now >> 32);
    //TRACE_6D(TRC_SCHED_BALLOON, v->domain->domain_id, v->vcpu_id, d1, d2, cpu, atomic_read(&v->pause_count));
    //SCHED_OP(VCPU2OP(v), unpause, v);
}
/***********************[end]***************************************/
