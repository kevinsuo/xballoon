...
static void evtchn_fifo_set_pending(struct vcpu *v, struct evtchn *evtchn)
{
     ...
     */
//TRACE_4D(TRC_SCHED_SETPENDING, v->domain->domain_id, v->vcpu_id, port, 2);
    if ( unlikely(!word) )
    {
        evtchn->pending = 1;
        return;
...
TRACE_5D(TRC_SCHED_SETPENDING, v->domain->domain_id, v->vcpu_id, port, 3, cpu);
         ...
         && !test_bit(EVTCHN_FIFO_LINKED, word) )
    {
        ...
        if ( !old_q ) {
//TRACE_4D(TRC_SCHED_SETPENDING, v->domain->domain_id, v->vcpu_id, port, 7);
            goto done;
        }
        ...
        {
            spin_unlock_irqrestore(&old_q->lock, flags);
//TRACE_4D(TRC_SCHED_SETPENDING, v->domain->domain_id, v->vcpu_id, port, 6);
            goto done;
        }
...
TRACE_5D(TRC_SCHED_SETPENDING, v->domain->domain_id, v->vcpu_id, port, 4, cpu);
            vcpu_mark_events_pending(v);
	}
    }
 done:
//TRACE_4D(TRC_SCHED_SETPENDING, v->domain->domain_id, v->vcpu_id, port, 5);
    if ( !was_pending )
