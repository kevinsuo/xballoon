...
static void evtchn_set_pending(struct vcpu *v, int port)
{
    ...
    d1 = (unsigned)(now >> 32);
    TRACE_6D(TRC_SCHED_PORT, v->domain->domain_id, v->vcpu_id, port, d1, d2, cpu);
...
void evtchn_check_pollers(struct domain *d, unsigned int port)
{
          ...
          vcpuid = find_next_bit(d->poll_mask, d->max_vcpus, vcpuid+1) )
    {
        ...
        {
            TRACE_3D(TRC_SCHED_POLLER, d->domain_id, v->vcpu_id, v->poll_evtchn);
            v->poll_evtchn = 0;
            vcpu_unblock(v);
        }
    }
