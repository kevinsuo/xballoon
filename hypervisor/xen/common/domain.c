    ...
    struct domain *d, unsigned int vcpu_id, unsigned int cpu_id)
{
	...
	//add by Kun
        v->runstate.prev_xBalloon_state = 0;
        v->runstate.xBalloon_state = 0;
        //atomic_set(&v->prev_xBalloon_state, 0);
        //atomic_set(&v->xBalloon_state, 0);
        v->xBalloon_sleep_time = 0;
        //end
...
void vcpu_pause_nosync(struct vcpu *v)
{
	...
	d1 = (unsigned)(now >> 32);
	TRACE_4D(TRC_SCHED_PAUSE, v->domain->domain_id, v->vcpu_id, d1, d2);
	atomic_inc(&v->pause_count);
	vcpu_sleep_nosync(v);
}
...
void vcpu_unpause(struct vcpu *v)
{
	...
	d2 = (unsigned)(now & 0xffffffffLL);
	d1 = (unsigned)(now >> 32);
	TRACE_4D(TRC_SCHED_UNPAUSE, v->domain->domain_id, v->vcpu_id, d1, d2);
...
TRACE_4D(TRC_SCHED_SETTIMER, v->domain->domain_id, v->vcpu_id, 111, 111);
        if ( v != current )
            return -EINVAL;

        if ( copy_from_guest(&set, arg, 1) )
            return -EFAULT;
    d2 = (unsigned)(set.timeout_abs_ns & 0xffffffffLL);
    d1 = (unsigned)(set.timeout_abs_ns >> 32);

TRACE_4D(TRC_SCHED_SETTIMER, v->domain->domain_id, v->vcpu_id, 222, 222);
        if ( (set.flags & VCPU_SSHOTTMR_future) &&
             (set.timeout_abs_ns < NOW()) )
            return -ETIME;

TRACE_4D(TRC_SCHED_SETTIMER, v->domain->domain_id, v->vcpu_id, d1, d2);
...
TRACE_4D(TRC_SCHED_SETTIMER, v->domain->domain_id, v->vcpu_id, 333, 333);
