...
#define CSCHED_TICKS_PER_TSLICE     30
...
//extern void _update_runstate_area(struct vcpu *v);
/*
 * Boot parameters
 */
...
csched_vcpu_sleep(const struct scheduler *ops, struct vcpu *vc)
{
   ...
   //if (local_events_need_delivery() && atomic_read(&vc->sleep_count))
if (vcpu_info(vc, evtchn_upcall_pending) && !vcpu_info(vc, evtchn_upcall_mask) && atomic_read(&vc->sleep_count))
    {
	    clear_bit(CSCHED_FLAG_VCPU_PARKED, &svc->flags);
	    atomic_dec(&vc->sleep_count);
	    atomic_dec(&vc->pause_count);
            //TRACE_5D(TRC_SCHED_CSLEEP, vc->domain->domain_id, vc->vcpu_id, test_bit(CSCHED_FLAG_VCPU_PARKED, &svc->flags), atomic_read(&vc->sleep_count), atomic_read(&vc->pause_count));
   	    return;
...
csched_acct(void* dummy)
{
    ...
    s_time_t now = NOW();
    uint32_t d1, d2;
    d2 = (unsigned)(now & 0xffffffffLL);
    d1 = (unsigned)(now >> 32);
    

    //TRACE_3D(TRC_SCHED_ACCT, smp_processor_id(), d1, d2);
    spin_lock_irqsave(&prv->lock, flags);
    now = NOW();
        d2 = (unsigned)(now & 0xffffffffLL);
        d1 = (unsigned)(now >> 32);
    //TRACE_3D(TRC_SCHED_ACCT1, smp_processor_id(), d1, d2);
    ...
    list_for_each_safe( iter_sdom, next_sdom, &prv->active_sdom )
    {
	    ...
	    if ( credit < 0 )
	    {
		    ...
		    svc->pri = CSCHED_PRI_TS_OVER;
                ...
                if ( sdom->cap != 0U &&
                     credit < -credit_cap &&
                     !test_bit(CSCHED_FLAG_VCPU_PARKED, &svc->flags) && !atomic_read(&svc->vcpu->sleep_count) )
                {
                    SCHED_STAT_CRANK(vcpu_park);
                    vcpu_pause_nosync(svc->vcpu);
                    set_bit(CSCHED_FLAG_VCPU_PARKED, &svc->flags); 
		    TRACE_6D(TRC_SCHED_CAP, svc->vcpu->domain->domain_id, svc->vcpu->vcpu_id, 0, d1, d2, -credit_cap);
                }
   		/* convert a xballoon sleep to capped pause */
		if (atomic_read(&svc->vcpu->sleep_count))
			atomic_dec(&svc->vcpu->sleep_count);
    ...
    d1 = (unsigned)(now >> 32);
                    SCHED_STAT_CRANK(vcpu_unpark);
		    TRACE_5D(TRC_SCHED_CAP, svc->vcpu->domain->domain_id, svc->vcpu->vcpu_id, 1, d1, d2);
                    vcpu_unpause(svc->vcpu);
		    clear_bit(CSCHED_FLAG_VCPU_PARKED, &svc->flags); 
                }
	    ...
	    if ( ! is_idle_domain(svc->vcpu->domain) ) {
                if (!test_bit(CSCHED_FLAG_VCPU_PARKED, &svc->flags)) {
                        svc->vcpu->xBalloon_sleep_time = 0;
			svc->vcpu->vcpu_allocate_time = NOW(); 

			//xBalloon_state = atomic_read(&svc->vcpu->xBalloon_state);
			//atomic_set(&svc->vcpu->prev_xBalloon_state, xBalloon_state);
			//atomic_set(&svc->vcpu->xBalloon_state, 1);
    now = NOW();
    d2 = (unsigned)(now & 0xffffffffLL);
    d1 = (unsigned)(now >> 32);
 
    TRACE_6D(TRC_SCHED_KUN_1, 	svc->vcpu->domain->domain_id, 
			      	svc->vcpu->vcpu_id, 
				svc->vcpu->runstate.xBalloon_state, 
				svc->vcpu->xBalloon_sleep_time,
				d1, d2);

                        svc->vcpu->runstate.prev_xBalloon_state = svc->vcpu->runstate.xBalloon_state;
                        svc->vcpu->runstate.xBalloon_state = 1;
    ...
    TRACE_6D(TRC_SCHED_KUN_1, 	svc->vcpu->domain->domain_id, 
			...
			//if(cap != 100) {
				send_guest_vcpu_virq(svc->vcpu, VIRQ_XBL);
		//	}
		}
	    }
//	    if (!is_idle_domain(svc->vcpu->domain) && (svc->vcpu->domain->domain_id != 0)){
//
//		_update_runstate_area(svc->vcpu);
//		ret = svc->vcpu->arch.pv_vcpu.need_update_runstate_area;
//		TRACE_3D(TRC_SCHED_COPY, svc->vcpu->domain->domain_id, svc->vcpu->vcpu_id, ret);
		//send_guest_vcpu_virq(svc->vcpu, VIRQ_XBL);
//	    }
...
csched_tick(void *_cpu)
{
    ...
    spc->tick++;
//TRACE_4D(TRC_SCHED_TICK, cpu, d1, d2, spc->tick);
...
//TRACE_3D(TRC_SCHED_TICKSUS, smp_processor_id(), d1, d2);
}
...
static void csched_tick_resume(const struct scheduler *ops, unsigned int cpu)
{
    ...
    set_timer(&spc->ticker, now + MICROSECS(prv->tick_period_us)
            - now % MICROSECS(prv->tick_period_us) );
//TRACE_3D(TRC_SCHED_TICKRES, smp_processor_id(), d1, d2);
}
...
static void csched_pause(const struct scheduler *ops, struct vcpu *v)
{
	...
	if (atomic_read(&v->pause_count) || local_events_need_delivery())
     		return;
        spin_lock_irqsave(&prv->lock, flags);
...
//	if (test_bit(0, (unsigned long *)&vcpu_info(v, evtchn_upcall_pending))){
	if (local_events_need_delivery()){
		clear_bit(CSCHED_FLAG_VCPU_PARKED, &svc->flags); 
		atomic_dec(&v->sleep_count);
		goto end;
	}
        //vcpu_pause_nosync(svc->vcpu);
	  vcpu_pause_fast(svc->vcpu);
end:
	//return;
        spin_unlock_irqrestore(&prv->lock, flags);
}
...
//add by Jia
static int csched_cap(const struct scheduler *ops, struct vcpu *v)
{
	struct csched_private *prv = CSCHED_PRIV(ops);
	struct csched_vcpu * const svc = CSCHED_VCPU(v);
        s_time_t now, delta;
        int credits_left, credits_burn, slack;
        uint64_t val;
	int capped = 1;
        uint32_t d1, d2;

	slack = 5; // 5 credit slackness
        d2 = (unsigned)(prv->master_ticker.expires & 0xffffffffLL);
        d1 = (unsigned)(prv->master_ticker.expires >> 32);
	//if (svc->sdom->cap != 0U) {
		now  = NOW();
		delta = prv->master_ticker.expires - now;
		val = delta * CSCHED_CREDITS_PER_MSEC;
		do_div(val, MILLISECS(1));
		//credits_left = atomic_read(&svc->credit) - svc->credit_cap *2;
		credits_left = atomic_read(&svc->credit) + 150;
		credits_burn = val + slack;
		if (credits_left > credits_burn) 
			capped = 0;
		else {
			capped = 1;
			send_guest_vcpu_virq(svc->vcpu, VIRQ_XBL);
                }	
	TRACE_6D(TRC_SCHED_CCAP, v->domain->domain_id, v->vcpu_id, d1, d2, credits_left, credits_burn);
	//}
	return capped;
}
...
static void csched_clear_flag(const struct scheduler *ops, struct vcpu *vc)
{
