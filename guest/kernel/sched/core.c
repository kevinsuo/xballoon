...
static DECLARE_WAIT_QUEUE_HEAD(xBalloon_sleep_queue);
//end
...
static void __sched_fork(unsigned long clone_flags, struct task_struct *p)
{
	...
	//Mod by Jia: preset differential clock to clock_task
	p->diff_clock = 0;
	p->decay = 0;
}
...
//                      //if (in_serving_softirq() || in_interrupt() || in_softirq() || in_irq())
...
need_resched:
	...
	sched_preempt_enable_no_resched();
        //mod by Jia
        #if 0
	if (strcmp(prev->comm, "netserver") == 0){
		this_cpu_ptr(&xen_runstate)->xBalloon_state = 1;
		wake_up_interruptible(&xBalloon_sleep_queue);
		if (need_resched())
			goto need_resched;
	}
	if (strcmp(prev->comm, "fread") == 0){
		this_cpu_ptr(&xen_runstate)->xBalloon_state = 1;
		wake_up_interruptible(&xBalloon_sleep_queue);
		if (need_resched())
			goto need_resched;
...
//add by Kun
void wake_xballoon_wq()
{
	int state = HYPERVISOR_sched_op(SCHEDOP_wake, NULL);
...
SYSCALL_DEFINE1(sched_get_clock, pid_t, pid)
{
	struct task_struct *p;
...
SYSCALL_DEFINE2(sched_set_clock, pid_t, pid, int, clock)
{
	struct task_struct *p;
	p = find_task_by_vpid(pid);
	if (unlikely(!p)) 
...
SYSCALL_DEFINE2(sched_set_decay, pid_t, pid, int, decay)
{
	struct task_struct *p;
	p = find_task_by_vpid(pid);
	if (unlikely(!p)) 
...
void __init sched_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		init_rt_rq(&rq->rt, rq);
		init_dl_rq(&rq->dl, rq);
                /* Mod by Jia: initialized steal and count */
                rq->steal = 0;
#ifdef CONFIG_FAIR_GROUP_SCHED
